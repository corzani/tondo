...
const tondoMock = (data) => {
  ...
  return ({
    tondo: Tondo(axios, 'CLIENT_ID_TEST', wallpaperService, downloadService, log),
    axios,
    log,
    setWallpaper,
    downloadService
...
describe("Tondo", function () {
  it("should call setWallpaper", async function () {
    ...
    const { axios, tondo, downloadService, setWallpaper, log } = tondoMock(randomPicture);
    ...
    expect(downloadService).toHaveBeenCalledWith(jasmine.anything(), randomPicture, resolve('./', 'Dwu85P9SOIk.jpg'), 'CLIENT_ID_TEST');
  ...
  it("should call downloadAllToFolder", async function () {
    ...
    expect(downloadService).toHaveBeenCalledWith(jasmine.anything(), randomPictures[0], resolve('./', 'Dwu85P9SOIk.jpg'), 'CLIENT_ID_TEST');
    expect(downloadService).toHaveBeenCalledWith(jasmine.anything(), randomPictures[1], resolve('./', 'Mwu55P9SOIk.jpg'), 'CLIENT_ID_TEST');
